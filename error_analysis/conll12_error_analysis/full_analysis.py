...
MAX_LEN = 200
CONLL05_GOLD_SYNTAX = 'gold-parse.txt'
CONLL05_GOLD_SRL = 'gold-props.txt'
...
if __name__ == '__main__':
  ...
  for s0, props, syn, g0, p0 in izip(sentences, predicates, syn_spans, gold, predicted):
    #print props, g0, p0
    ...
    #print s0
      ...
      uv, cv, rv = compute_srl_violations(gold_spans)
      #if rv[0] > 0:
      #  print s0
      #  print g0

      # Compute SRL violations
      add_to_histogram(gold_violations, compute_srl_violations(gold_spans))
      add_to_histogram(pred_violations, compute_srl_violations(pred_spans))
      ...
      p0, r0 = breakdown_acc_by_arg_length(gold_spans, gold_matched, pred_spans, pred_matched)
      add_to_histogram(p_arglen, p0)
      add_to_histogram(r_arglen, r0)

      p0, r0 = breakdown_acc_by_dist_to_prop(gold_spans, gold_matched, pred_spans, pred_matched, prop_id)
      add_to_histogram(p_propdist, p0)
      add_to_histogram(r_propdist, r0)

      p0, r0 = breakdown_acc_by_dist_to_prop(gold_spans, gold_matched, pred_spans, pred_matched, 0)
      add_to_histogram(p_sdist, p0)
      add_to_histogram(r_sdist, r0)

      p0, r0 = breakdown_acc_by_num_intv_args(gold_spans, gold_matched, pred_spans, pred_matched, 0)
      add_to_histogram(p_iargs, p0)
      add_to_histogram(r_iargs, r0)

      p0, r0 = breakdown_acc_by_slen(gold_spans, gold_matched, pred_spans, pred_matched, len(s0))
      add_to_histogram(p_slen, p0)
      add_to_histogram(r_slen, r0)

      # Compute syntax match
      m,p = compute_syntax_match(gold_spans, syn)
      gold_synmatch[0] += m
      gold_synmatch[1] += p
      m,p = compute_syntax_match(pred_spans, syn)
      pred_synmatch[0] += m
      pred_synmatch[1] += p


  precision = 1.0 * num_matched / num_predicted
  recall = 1.0 * num_matched / num_gold
  f1 = 2.0 * precision * recall / (precision + recall)

  print 'F1', precision, recall, f1
  print 'Gold SRL violations', gold_violations, [100.0 * g[0] / g[1] for g in gold_violations]
  print 'Pred SRL violations', pred_violations, [100.0 * p[0] / p[1] for p in pred_violations]

  #print_histogram('argument length', p_arglen, r_arglen, [(0,1), (2,4), (5,200)])
  print_histogram('distance to predicate', p_propdist, r_propdist, [(0,1), (2,3),(4,7),(8,200)])
  print_histogram('sentence length', p_slen, r_slen, [(0,10), (11,20),(21,30), (31,40), (41,300)])

  #print_histogram('num. intv. args', p_iargs, r_iargs, [(0,0), (1,1), (2,200)])
  #print_histogram('distance to sentence start', p_sdist, r_sdist, 30)

  print 'Percentage of gold arguments matching syntax: {} ({}/{})'\
          .format(100.0 * gold_synmatch[0] / gold_synmatch[1], gold_synmatch[0], gold_synmatch[1])

  print 'Percentage of predicted arguments matching syntax: {} ({}/{})'\
          .format(100.0 * pred_synmatch[0] / pred_synmatch[1], pred_synmatch[0], pred_synmatch[1])
